...
onMounted(() => {
  ...
  map.fitBounds([
    [45.77579087036024, 7.899644901157423],
    [49.981425155587345, 19.77734080646181]
  ])

  L.tileLayer(`https://api.maptiler.com/maps/basic-v2/{z}/{x}/{y}${scale}.png?key=` + apiKey, {
    tileSize: 512,
    zoomOffset: -1,
    minZoom: 7,
    maxZoom: 19,
  ...
  const addCastles = async () => {
    ...
    const castles = L.geoJSON(data, {
      onEachFeature: function (feature, layer) {
        ...
        if (
          feature.properties.castle_type === 'defensive' ||
          feature.properties.castle_type === 'defensive;stately'
        ) {
...
<template>
  ...
  <InfoBox
    v-if="infoboxVisible"
    @close="infoboxVisible = false"
    :clickedCastle="clickedCastle"
